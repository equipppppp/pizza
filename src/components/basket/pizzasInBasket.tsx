...
import PizzaInBasket from "./pizzaInBasket/pizzaInBasket";
import MainButton from "components/UI/button/mainButton";

import styles from "./pizzasInBasket.module.scss";
    ...
    <>
      <div className={styles.container}>
        <div className={styles.header}>
          ...
          <div className={styles.clearBasket} onClick={onClickClearBasket}>
            <svg
              width="20"
              height="20"
              viewBox="0 0 20 20"
        ...
        {pizzasInBasket.map((pizza) => (
          <PizzaInBasket
            key={`${pizza.name}+${pizza.size}+${pizza.type}`}
            {...pizza}
          />
        ))}
        ...
        <div className={styles.footer}>
          <Link to="/pizza">
            <MainButton grey>Вернуться назад</MainButton>
          </Link>
          <MainButton handleClick={openModal}>Перейти к оформлению</MainButton>
        </div>
      </div>
    </>
  );
